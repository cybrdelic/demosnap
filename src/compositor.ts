...
export async function compose(options: ComposeOptions) {
  ...
  page.on('console', (msg: any) => {
    try {
      console.log('[compose page]', msg.type(), msg.text());
    } catch {}
  });
  // Attach request/response listeners BEFORE navigation to capture compositor.js request
  page.on(
    'request',
    (req: any) => options.debug && console.log('[compose req early]', req.method(), req.url())
  );
  page.on(
    'response',
    (res: any) => options.debug && console.log('[compose res early]', res.status(), res.url())
  );
  // Derive base origin from videoUrl (http://localhost:PORT)
  let origin = '';
  try {
    const u = new URL(options.videoUrl);
    origin = `${u.protocol}//${u.host}`;
  } catch {}
  const debug = options.debug ? '&debug=1' : '';
  const effBitrate = options.videoBitrateKbps || options.bitrateKbps || '';
  const hudMode = options.hud || 'minimal';
  // Provide a default aggressive camera action style unless caller overrides via theme semantics later.
  const camStyle = 'aggressive';
  const extra = `&bitrate=${encodeURIComponent(String(effBitrate))}&link=${encodeURIComponent(options.link || '')}&fps=${encodeURIComponent(String(options.fps || ''))}&quality=${encodeURIComponent(options.quality || '')}&letterbox=${options.letterbox === false ? '0' : '1'}&hud=${encodeURIComponent(hudMode)}&cam=${encodeURIComponent(camStyle)}`;
  const url =
    origin +
    `/compositor?video=${encodeURIComponent(options.videoUrl)}&title=${encodeURIComponent(options.title ?? '')}&subtitle=${encodeURIComponent(options.subtitle ?? '')}&theme=${encodeURIComponent(options.theme ?? 'sky')}&timeline=${encodeURIComponent(options.timelineBase64 ?? '')}&fallbackDuration=${encodeURIComponent(String(options.duration || 0))}${extra}${debug}`;
  console.log('[compose] navigate URL', url);
  let nodeDoneCalled = false;
  let nodeDoneMeta: any = null;
  // Expose nodeDone BEFORE navigation so page scripts can call immediately
  await page.exposeFunction(
    'nodeDone',
    async (blobBase64: string, coverBase64: string, metaJson?: string) => {
      try {
        if (blobBase64) {
        ...
        if (metaJson) {
          try {
            nodeDoneMeta = JSON.parse(metaJson);
          } catch {}
        }
        nodeDoneCalled = true;
        console.log('[compose] nodeDone wrote artifacts', {
          ...
          meta: nodeDoneMeta || null,
        });
      } catch (e) {
        console.error('[compose] nodeDone error', e);
      }
    }
  );
  ...
  page.on(
    'request',
    (req: any) => options.debug && console.log('[compose req]', req.method(), req.url())
  );
  page.on('requestfailed', (req: any) =>
    console.warn('[compose req FAILED]', req.failure()?.errorText, req.url())
  );
  page.on('response', async (res: any) => {
    if (options.debug) {
      console.log('[compose res]', res.status(), res.url());
    }
  });
  ...
  try {
    ...
    console.warn('[compose] eval page info failed', e);
  }
  const maxWait = Math.max(6000, options.duration + 4000);
  console.log('[compose] waiting for __COMPOSITOR_BOOT');
  await page
    .waitForFunction(() => (window as any).__COMPOSITOR_BOOT === true, { timeout: 10000 })
    .then(() => console.log('[compose] __COMPOSITOR_BOOT detected'))
    .catch(() => console.warn('[compose] __COMPOSITOR_BOOT timeout'));
  ...
  if (!bootPresent) {
    ...
    try {
      ...
      console.warn('[compose] failed DOM snapshot', e);
    }
    await page.evaluate(() => {
      (window as any).__COMPOSITOR_BOOT = true;
    });
  }
  console.log('[compose] waiting for COMPOSITOR_READY up to', maxWait, 'ms');
  const startWait = Date.now();
  await page
    .waitForFunction(() => (window as any).COMPOSITOR_READY === true, { timeout: maxWait })
    .then(() => {
  ...
  // Wait for recording completion flag with buffer (duration + 3.5s)
  const recWait = options.duration + 3500;
  const recStart = Date.now();
  console.log('[compose] waiting for __RECORDING_COMPLETE up to', recWait, 'ms');
  await page
    .waitForFunction(() => (window as any).__RECORDING_COMPLETE === true, { timeout: recWait })
    .then(() => console.log('[compose] __RECORDING_COMPLETE after', Date.now() - recStart, 'ms'))
    .catch(() =>
      console.warn('[compose] __RECORDING_COMPLETE timeout after', Date.now() - recStart, 'ms')
    );
  ...
  if (!fs.existsSync(options.coverPath) || fs.statSync(options.coverPath).size === 0) {
    try {
      ...
      console.warn('[compose] cover screenshot fallback failed', e);
    }
  }
  const pv = page.video();
  await page.close();
  ...
  if ((!fs.existsSync(options.outVideo) || fs.statSync(options.outVideo).size === 0) && pv) {
    try {
      ...
      if (fs.existsSync(p)) {
        ...
        console.log(
          '[compose] wrote composed video from playwright record (fallback, nodeDone not used)',
          p
        );
      }
    } catch (e) {
      console.error('[compose] failed copying playwright video', e);
    }
  }
  // Write meta diagnostics
  try {
    const metaPath = path.join(path.dirname(options.outVideo), 'compose-meta.json');
    const meta: any = {
      nodeDoneCalled,
      fallbackUsed: !nodeDoneCalled,
      expectedDurationMs: options.duration,
      timestamp: new Date().toISOString(),
    };
    if (nodeDoneMeta) meta.nodeDoneMeta = nodeDoneMeta;
    if (fs.existsSync(options.outVideo)) meta.outputBytes = fs.statSync(options.outVideo).size;
    fs.writeFileSync(metaPath, JSON.stringify(meta, null, 2));
  } catch (e) {
    console.warn('[compose] failed writing compose-meta.json', e);
  }
  await browser.close();
}
